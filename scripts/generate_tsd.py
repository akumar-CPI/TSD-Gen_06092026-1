...
def render_schema_table(doc, schema, name, properties, elem_id=""):
    """Render one instance's table: only the fields the schema defines,
    plus any nested CPI table properties relevant to this element."""
...
def render(data, out_path):
    ...
    for resolved_type, elements in groups.items():
        ...
        for elem in elements:
            if schema:
                render_schema_table(doc, schema, elem["name"], elem["properties"], elem_id=elem.get("id", ""))
            else:
